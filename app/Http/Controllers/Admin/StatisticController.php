        ...
        public function index(Request $request)
        {
            ...
            $data = Cache::remember($cacheKey, $cacheDuration, function () use ($request) {
                ...
                foreach ($levels as $initialLevel) {
                    $levelTotals[data_get($initialLevel, 'name')] = 0;
                    foreach ($levels as $finalLevel) {
                        $levelEffectivenessMatrix[data_get($initialLevel, 'name')][data_get($finalLevel, 'name')] = 0;
                    }
                }
                ...
                foreach ($levelProgressionData as $data) {
                    ...
                    if ($finalLevel && isset($levelEffectivenessMatrix[$initialLevelName][data_get($finalLevel, 'name')])) {
                        $levelEffectivenessMatrix[$initialLevelName][data_get($finalLevel, 'name')]++;
                        $levelTotals[$initialLevelName]++;
                            ...
                            $levelOrder = array_column($levels->toArray(), 'name');
