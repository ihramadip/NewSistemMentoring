<x-app-layout>
    <x-slot name="header">
        <x-page-header title="{{ __('Manajemen Kelompok Mentoring') }}" subtitle="Kelola daftar kelompok mentoring, mentor, dan anggotanya.">
            <x-slot name="icon">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-medium text-gray-900">Daftar Kelompok Mentoring</h3>
                        <a href="{{ route('admin.mentoring-groups.create') }}" class="inline-flex items-center px-4 py-2 bg-brand-teal border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-brand-gold active:bg-brand-gold focus:outline-none focus:ring-2 focus:ring-brand-teal focus:ring-offset-2 transition ease-in-out duration-150">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($mentoringGroups as $group)
                                    <tr class="@if($loop->even) bg-brand-mist/20 @endif hover:bg-brand-mist/40">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $group->mentor->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $group->level->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $group->schedule_info }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.mentoring-groups.edit', $group) }}" class="text-brand-teal hover:text-brand-gold mr-3">Edit</a>
                                            <form action="{{ route('admin.mentoring-groups.destroy', $group) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Apakah Anda yakin ingin menghapus kelompok ini?')">Hapus</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
