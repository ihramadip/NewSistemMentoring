<section id="dokumentasi" class="bg-white px-6 py-24">
    <div class="mx-auto max-w-6xl">
        <div class="flex flex-col items-start justify-between gap-6 md:flex-row">
            <div>
                <p class="text-sm font-semibold uppercase tracking-[0.6em] text-brand-teal">Dokumentasi Kegiatan</p>
                <h3 class="section-title font-heading text-4xl text-brand-ink">Cerita visual yang konsisten.</h3>
            </div>
        ...
        <div class="mt-10">
            <div class="flex space-x-6 overflow-x-auto pb-4 scrollbar-hide">
                @foreach ($documentation as $doc)
                    <div class="flex-shrink-0 w-80 overflow-hidden rounded-3xl border border-brand-sky/40">
                        ...
                        <div class="p-5">
                            ...
                            <h4 class="mt-3 font-heading text-xl text-brand-ink">{{ $doc['title'] }}</h4>
                            <p class="mt-2 text-sm text-slate-600">{{ $doc['description'] }}</p>
                            <button class="mt-4 text-sm font-semibold text-brand-teal">Lihat galeri →</button>
                        </div>
                    </div>
                @endforeach
            </div>
